"use client";

import {
  forwardRef,
  useCallback,
  useEffect,
  useImperativeHandle,
  useRef,
  useState,
} from "react";
import { injectBridgeIntoHtml } from "@/lib/bridge-script";
import type { TaskAnswers } from "@/types";

type SaveStatus = "idle" | "saving" | "saved" | "error";

export type SaveOptions = {
  keepalive?: boolean;
};

export type TaskIframeHandle = {
  flushSave: () => Promise<void>;
};

interface TaskIframeProps {
  htmlContent: string;
  initialAnswers: TaskAnswers;
  restoreKey: string;
  onAnswersChange: (
    answers: TaskAnswers,
    options?: SaveOptions
  ) => Promise<void>;
  readOnly?: boolean;
  saveStatusLabel?: {
    saving: string;
    saved: string;
    error: string;
  };
  showSaveIndicator?: boolean;
  fullscreen?: boolean;
}

export const TaskIframe = forwardRef<TaskIframeHandle, TaskIframeProps>(
  function TaskIframe(
    {
      htmlContent,
      initialAnswers,
      restoreKey,
      onAnswersChange,
      readOnly = false,
      saveStatusLabel,
      showSaveIndicator = true,
      fullscreen = false,
    },
    ref
  ) {
    const iframeRef = useRef<HTMLIFrameElement>(null);
    const [isReady, setIsReady] = useState(false);
    const [saveStatus, setSaveStatus] = useState<SaveStatus>("idle");
    const answersRef = useRef<TaskAnswers>(initialAnswers);
    const saveTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
    const saveQueueRef = useRef<Promise<void>>(Promise.resolve());
    const restoredForKeyRef = useRef<string | null>(null);
    const pendingFlushRef = useRef<{
      resolve: () => void;
      reject: (error: Error) => void;
    } | null>(null);
    const isPageHidingRef = useRef(false);

    const srcDoc = injectBridgeIntoHtml(htmlContent);

    const sendToIframe = useCallback((message: object) => {
      iframeRef.current?.contentWindow?.postMessage(message, "*");
    }, []);

    const enqueueSave = useCallback(
      (answers: TaskAnswers, options?: SaveOptions) => {
        saveQueueRef.current = saveQueueRef.current
          .catch(() => {})
          .then(async () => {
            setSaveStatus("saving");
            try {
              await onAnswersChange(answers, options);
              setSaveStatus("saved");
              if (saveTimeoutRef.current) clearTimeout(saveTimeoutRef.current);
              saveTimeoutRef.current = setTimeout(
                () => setSaveStatus("idle"),
                2000
              );
            } catch {
              setSaveStatus("error");
              throw new Error("Save failed");
            }
          });
        return saveQueueRef.current;
      },
      [onAnswersChange]
    );

    const restoreAnswers = useCallback(() => {
      if (restoredForKeyRef.current === restoreKey) return;
      sendToIframe({ type: "RESTORE_ANSWERS", answers: initialAnswers });
      sendToIframe({ type: "SET_READONLY", readonly: readOnly });
      answersRef.current = initialAnswers;
      restoredForKeyRef.current = restoreKey;
    }, [initialAnswers, readOnly, restoreKey, sendToIframe]);

    useImperativeHandle(
      ref,
      () => ({
        flushSave: () =>
          new Promise<void>((resolve, reject) => {
            pendingFlushRef.current = { resolve, reject };
            sendToIframe({ type: "REQUEST_ANSWERS" });

            setTimeout(() => {
              if (!pendingFlushRef.current) return;
              const flush = pendingFlushRef.current;
              pendingFlushRef.current = null;
              saveQueueRef.current
                .then(() => flush.resolve())
                .catch((error) =>
                  flush.reject(
                    error instanceof Error ? error : new Error("Save failed")
                  )
                );
            }, 5000);
          }),
      }),
      [sendToIframe]
    );

    useEffect(() => {
      if (isReady) {
        restoreAnswers();
      }
    }, [isReady, restoreAnswers]);

    useEffect(() => {
      if (isReady) {
        sendToIframe({ type: "SET_READONLY", readonly: readOnly });
      }
    }, [readOnly, isReady, sendToIframe]);

    useEffect(() => {
      const flushSave = () => {
        isPageHidingRef.current = true;
        sendToIframe({ type: "REQUEST_ANSWERS" });
      };

      window.addEventListener("pagehide", flushSave);
      window.addEventListener("beforeunload", flushSave);

      const handleMessage = (event: MessageEvent) => {
        if (event.source !== iframeRef.current?.contentWindow) return;

        const data = event.data;
        if (!data || typeof data !== "object") return;

        if (data.type === "TASK_READY") {
          restoredForKeyRef.current = null;
          setIsReady(true);
          sendToIframe({
            type: "RESTORE_ANSWERS",
            answers: answersRef.current,
          });
          sendToIframe({ type: "SET_READONLY", readonly: readOnly });
          return;
        }

        if (data.type === "ANSWERS_CHANGED") {
          const answers = data.answers as TaskAnswers;
          answersRef.current = answers;

          const isFlush = !!pendingFlushRef.current;
          const shouldSave = !readOnly || isFlush;
          if (!shouldSave) return;

          const savePromise = enqueueSave(answers, {
            keepalive: isFlush || isPageHidingRef.current,
          });
          isPageHidingRef.current = false;

          if (pendingFlushRef.current) {
            const flush = pendingFlushRef.current;
            pendingFlushRef.current = null;
            savePromise
              .then(() => flush.resolve())
              .catch((error) =>
                flush.reject(
                  error instanceof Error ? error : new Error("Save failed")
                )
              );
          }
        }
      };

      window.addEventListener("message", handleMessage);
      return () => {
        window.removeEventListener("message", handleMessage);
        window.removeEventListener("pagehide", flushSave);
        window.removeEventListener("beforeunload", flushSave);
        if (saveTimeoutRef.current) clearTimeout(saveTimeoutRef.current);
      };
    }, [enqueueSave, readOnly, sendToIframe]);

    return (
      <div
        className={
          fullscreen
            ? "relative flex h-full min-h-0 flex-1 flex-col"
            : "space-y-2"
        }
      >
        {showSaveIndicator && saveStatusLabel && saveStatus !== "idle" && (
          <div
            className={
              fullscreen
                ? "absolute right-3 top-3 z-10 rounded-md bg-white/90 px-2 py-1 text-xs shadow-sm"
                : "text-sm text-gray-500"
            }
          >
            {saveStatus === "saving" && saveStatusLabel.saving}
            {saveStatus === "saved" && (
              <span className="text-green-600">{saveStatusLabel.saved}</span>
            )}
            {saveStatus === "error" && (
              <span className="text-red-600">{saveStatusLabel.error}</span>
            )}
          </div>
        )}
        <iframe
          ref={iframeRef}
          srcDoc={srcDoc}
          sandbox="allow-scripts allow-forms allow-same-origin"
          className={
            fullscreen
              ? "h-full w-full flex-1 border-0 bg-white"
              : "w-full min-h-[500px] rounded-md border border-gray-200 bg-white"
          }
          title="Task"
        />
      </div>
    );
  }
);
